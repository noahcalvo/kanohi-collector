...
import { DUPLICATE_ESSENCE_BY_RARITY, MAX_LEVEL_BY_RARITY } from "../../../../lib/constants";
...
export function StrongestMasks({
  ...
  const calculateStrength = (mask: CollectionMask): number => {
    return (mask.level - 1) * MAX_LEVEL_BY_RARITY[mask.rarity] + (mask.essence || 0) + DUPLICATE_ESSENCE_BY_RARITY[mask.rarity];
  };
  ...
  return (
    <div className="flex flex-col p-6 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300 relative overflow-hidden md:col-span-2">
      {/* Subtle accent glow */}
      ...
      <div className="flex justify-center items-end gap-6 relative z-10">
        ...
        {topMasks[1] && (
          <div className="flex flex-col items-center">
            <div className="relative w-24 h-24 mb-3 rounded-xl bg-white/40 backdrop-blur-sm p-1">
              <ColoredMask
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-slate-600 font-semibold truncate w-28">{topMasks[1].name}</div>
              <div className="text-xs text-slate-500 font-light">Lvl {topMasks[1].level}</div>
            </div>
          </div>
        )}
        ...
        {topMasks[0] && (
          <div className="flex flex-col items-center">
            <div className="relative w-32 h-32 mb-4 p-1.5">
              <ColoredMask
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-slate-700 truncate w-40 font-bold">{topMasks[0].name}</div>
              <div className="text-sm text-slate-500 font-light">Lvl {topMasks[0].level}</div>
            </div>
          </div>
        )}
        ...
        {topMasks[2] && (
          <div className="flex flex-col items-center">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-slate-600 font-semibold truncate w-28">{topMasks[2].name}</div>
              <div className="text-xs text-slate-500 font-light">Lvl {topMasks[2].level}</div>
            </div>
          </div>
        )}
      </div>
