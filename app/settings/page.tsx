...
export default async function SettingsPage() {
  ...
  return (
    <div className="card max-w-lg mx-auto mt-8">
      ...
      <div className="mb-2">
        <span className="font-semibold">Account Age:</span> {createdAt ? `${ageDays} days` : "Unknown"}
      </div>
      {isGuest ? (
        <div className="mt-4 text-yellow-800 bg-yellow-50 border border-yellow-200 rounded p-3">
          You are playing as a guest. <Link href="/sign-up" className="underline">Create an account</Link> to save your progress.
        </div>
      ) : (
        <div className="mt-4">
          <SignOutButton />
