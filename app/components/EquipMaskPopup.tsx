...
export default function EquipMaskPopup({
  ...
  onEquip: (maskId: string, slot: EquipSlot, color?: string, transparent?: boolean) => void;
  onClose: () => void;
  currentToaEquipped?: { maskId: string; name: string; color?: string; transparent?: boolean } | null;
  currentTuragaEquipped?: { maskId: string; name: string; color?: string; transparent?: boolean } | null;
}) {
  ...
  return (
    <div ref={popupRef} className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-white rounded-xl shadow-xl border border-slate-200 p-4 min-w-[252px] z-20">
      <div className="text-xs font-semibold text-slate-700 mb-3 text-center">
        Choose Slot
      </div>
      {/* Clickable options */}
      <div className="grid grid-cols-2 gap-x-3 gap-y-1">
        <div className="ml-2 text-[10px] uppercase font-semibold text-slate-500">Toa</div>
        <div className="ml-2 text-[10px] uppercase font-semibold text-slate-500">Matoran</div>
        <button
          type="button"
        ...
        >
          <div className="pointer-events-none absolute inset-0 bg-black/60 backdrop-blur-[2px] opacity-0 group-hover/equip:opacity-100 transition-opacity flex items-center justify-center z-10">
            <span className="text-white text-xs font-bold tracking-wide">Swap</span>
          </div>
