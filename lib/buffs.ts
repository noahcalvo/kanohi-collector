...
function maskBuffValue(mask: UserMask): BuffTotals {
  ...
  if (!def) return { pack_luck: 0, timer_speed: 0, duplicate_eff: 0, discovery: 0 };
  const slotMultiplier = mask.equipped_slot === "TURAGA" ? 0.5 : mask.equipped_slot === "TOA" ? 1 : 0;
  const value = def.buff_base_value * mask.level * slotMultiplier;
  const zero: BuffTotals = { pack_luck: 0, timer_speed: 0, duplicate_eff: 0, discovery: 0 };
...
export function computeBuffs(userId: string): BuffTotals {
  ...
  const totals = masks.reduce<BuffTotals>((acc, mask) => {
